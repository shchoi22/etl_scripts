...
try:
    start = datetime.now()
    f = open("/home/schoi/etl/queries/application_approvals_public.sql",'r')
    ...
    data2.to_csv('application_approvals'+'output.csv', sep=';', na_rep='', cols=None, header=False, index=False)

    cur_chartio.execute("TRUNCATE TABLE " + 'public.application_approvals')
    #con_chartio.commit()
    cur_chartio.execute("DROP TABLE IF EXISTS " + 'public.application_approvals')
    #con_chartio.commit()
    cur_chartio.execute("CREATE TABLE " + 'public.application_approvals' +'()')
    con_chartio.commit()

    for column in data2.columns:
        cur_chartio.execute("ALTER TABLE public.application_approvals  ADD COLUMN " + column.lower() + " text;")
        con_chartio.commit()

    columnString = ','.join(data2.columns)

    cur_chartio.copy_from(open('application_approvals'+'output.csv','r'), 'public.application_approvals', sep=';', null = 'NA', columns=None)
    con_chartio.commit()

    cur_chartio.execute("GRANT ALL ON TABLE public.application_approvals TO GROUP reporting_role;")
    con_chartio.commit()
    ...
    sender = cf.gmail_id
    receivers = [cf.recip]
    message = """From: Linux Box <{0}>\nTo:<{1}>\nSubject: ERROR LOG-FAIL TO WRITE {2}\n\nError message: {3}.""".format(cf.gmail_id,cf.recip,'application_approvals_analytics', error_log)
    try:
